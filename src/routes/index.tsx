...
function Studio() {
  ...
  return <div className="flex h-screen min-h-0 flex-col overflow-hidden bg-background">
    <header className="shrink-0 border-b border-border bg-surface"><CmykBar /><div className="flex flex-wrap items-center justify-between gap-3 px-6 py-4"><div className="flex items-center gap-4"><img src="/logo-black.svg" alt="Imprima Cooper" className="h-9 w-auto max-w-[220px] object-contain" /><span className="text-sm uppercase tracking-[0.14em] text-muted-foreground">Gerador de Blocos</span></div><div className="flex items-center gap-3"><Btn onClick={() => newBlock(doc.typeId)}>Novo bloco</Btn><PdfExport /></div></div></header>
