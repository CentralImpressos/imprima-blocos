...
function roundedRectPath(x: number, y: number, w: number, h: number, radius: number, corners: RectCorner[] = ["tl", "tr", "br", "bl"]) {
  const r = Math.min(radius, w / 2, h / 2); const tl = corners.includes("tl"), tr = corners.includes("tr"), br = corners.includes("br"), bl = corners.includes("bl");
...
export function DocumentPreview() {
  ...
  return <div className="flex h-full min-h-0 flex-col overflow-hidden bg-background p-4 pb-3">
    <div className="flex shrink-0 flex-wrap items-center justify-between gap-3 pb-3"><div className="flex flex-wrap items-center gap-3 text-[11px] uppercase tracking-wider text-muted-foreground"><span className="font-semibold text-foreground">{type.label}</span><span>{size.label}</span><span>{size.widthMm} × {size.heightMm} mm</span><span>{doc.copies} via(s)</span></div><div className="flex items-center gap-1.5"><button type="button" className={buttonClass(mode === "fit")} onClick={() => changeMode("fit")} title="Mostrar o documento inteiro">⊞ Ajustar</button><button type="button" className={buttonClass(mode === "width")} onClick={() => changeMode("width")} title="Preencher a largura disponível">↔ Largura</button><div className="ml-1 flex items-center overflow-hidden rounded-[3px] border border-border"><button type="button" className="px-2 py-1 text-sm font-semibold text-muted-foreground hover:bg-muted hover:text-foreground" onClick={() => changeZoom(zoom - 0.1)} aria-label="Diminuir zoom">−</button><button type="button" className="min-w-[58px] border-x border-border px-2 py-1 text-[11px] font-semibold uppercase tracking-wide text-foreground" onClick={() => changeZoom(1)} title="Zoom 100%">{Math.round(zoom * 100)}%</button><button type="button" className="px-2 py-1 text-sm font-semibold text-muted-foreground hover:bg-muted hover:text-foreground" onClick={() => changeZoom(zoom + 0.1)} aria-label="Aumentar zoom">+</button></div></div></div>
    <div ref={stageRef} className="min-h-0 min-w-0 flex-1 overflow-auto rounded-[3px] bg-background">
      <div className="flex min-h-full min-w-full items-center justify-center p-4" style={{ minWidth: isOversized ? `${previewWidth + 32}px` : undefined, minHeight: isOversized ? `${previewHeight + 32}px` : undefined }}><div className="relative shrink-0 shadow-[0_1px_2px_rgba(0,0,0,0.12)]" style={{ width: previewWidth || undefined, height: previewHeight || undefined }}><svg viewBox={`0 0 ${vw} ${vh}`} className="h-full w-full bg-white" style={{ outline: "1px solid var(--color-border-strong)" }}>{b > 0 && <rect x={0} y={0} width={vw} height={vh} fill="var(--color-accent)" opacity={0.5} />}<g transform={`translate(${b} ${b})`}><rect x={0} y={0} width={built.widthMm} height={built.heightMm} fill="#ffffff" />{built.guides.map((el, i) => <Element key={`g${i}`} el={el} i={i} />)}{built.content.map((el, i) => <Element key={`c${i}`} el={el} i={i} />)}{built.production.map((el, i) => <Element key={`p${i}`} el={el} i={i} />)}{built.stubY !== null && <text x={built.widthMm - 2} y={built.stubY - 1} fontSize={2.2} textAnchor="end" fill="var(--color-primary)" fontFamily="Inter, Arial, sans-serif">SERRILHA</text>}</g></svg></div></div>
    </div>
    <div className="flex shrink-0 flex-wrap items-center justify-center gap-x-5 gap-y-1 pt-2 leading-none text-[11px] font-medium uppercase tracking-wider text-muted-foreground">{production.showBleed && <span>Sangria {production.bleedMm} mm</span>}<span>Margem segura {production.safeMm} mm</span>{doc.serrilha && <span className="text-primary">Serrilha</span>}{doc.grampo && <span className="text-primary">Grampo</span>}{doc.canhoto && <span className="text-primary">Canhoto</span>}{doc.roundedCorners && <span className="text-primary">Cantos arredondados</span>}</div>
  </div>;
}
