    ...
    <div className="flex flex-col gap-3 p-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        <TextField
          ...
          placeholder="POLENTA'S"
          className="col-span-2"
        />
        <TextField
          label="Nome fantasia"
          value={company.tradeName}
          onChange={(v) => setCompany({ tradeName: v })}
          placeholder="POLENTA'S RESTAURANTE E PIZZARIA"
          className="col-span-2"
        />
        ...
        <TextField
          ...
          onChange={(v) => setCompany({ complement: v })}
        />
        ...
        <TextField
          ...
          onChange={(v) => setCompany({ instagram: v })}
        />
        <TextField
          label="Facebook"
          value={company.facebook}
          onChange={(v) => setCompany({ facebook: v })}
          className="col-span-2"
        />
        <Field label="Observações" className="col-span-2">
          <textarea
            className="field-input min-h-16 py-1.5"
            value={company.notes}
            onChange={(e) => setCompany({ notes: e.target.value })}
          />
        </Field>
        <TextField
          label="Texto padrão de rodapé"
      ...
      <p className="text-[10px] text-muted-foreground">
        Os dados da empresa são independentes do template e reaproveitados em todos os blocos.
      </p>
    </div>
  );
}
