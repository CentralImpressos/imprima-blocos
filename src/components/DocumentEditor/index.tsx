  ...
  return (
    <div className="flex flex-col gap-2 p-3">
      ...
      <div className="mt-1 flex flex-col gap-2 rounded-[3px] border border-border p-2.5">
        ...
        <label className="flex flex-col gap-1"><span className="text-[11px] font-medium text-muted-foreground">Fonte dos títulos</span><select className="field-input" value={doc.titleFont} onChange={(e) => setDoc({ titleFont: e.target.value })} style={{ fontFamily: doc.titleFont }}>{GOOGLE_FONTS.map((font) => <option key={font} value={font} style={{ fontFamily: font }}>{font}</option>)}</select></label>
        <SizeSlider label="Tamanho dos títulos" value={doc.titleSize} min={8} max={24} step={0.5} onChange={(v) => setDoc({ titleSize: v })} />
        <label className="flex flex-col gap-1"><span className="text-[11px] font-medium text-muted-foreground">Fonte do corpo</span><select className="field-input" value={doc.bodyFont} onChange={(e) => setDoc({ bodyFont: e.target.value })} style={{ fontFamily: doc.bodyFont }}>{GOOGLE_FONTS.map((font) => <option key={font} value={font} style={{ fontFamily: font }}>{font}</option>)}</select></label>
        <SizeSlider label="Tamanho do corpo" value={doc.bodySize} min={5} max={12} step={0.5} onChange={(v) => setDoc({ bodySize: v })} />
        <SizeSlider label="Tamanho da tabela" value={doc.table.fontSize} min={5} max={12} step={0.5} onChange={(v) => setDoc({ table: { ...doc.table, fontSize: v } })} />
      </div>
