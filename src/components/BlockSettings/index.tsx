...
export function BlockSettings() {
  const { doc, setDoc } = useStudio(); const rules = getType(doc.typeId).rules; const landscapeOnly = rules.canhoto === "required"; const availableSizes = landscapeOnly ? BLOCK_SIZES.filter((s) => s.widthMm > s.heightMm) : BLOCK_SIZES;
  useEffect(() => { if (landscapeOnly && getSize(doc.sizeId).widthMm <= getSize(doc.sizeId).heightMm) setDoc({ sizeId: getType(doc.typeId).defaultSizeId }); }, [doc.typeId, doc.sizeId, landscapeOnly, setDoc]);
  return <div className="flex flex-col gap-3 p-3">
    ...
    <div className="flex flex-col gap-1"><span className="panel-label">Vias</span><div className="flex gap-1">{COPIES_OPTIONS.map((o) => <button key={o.value} onClick={() => setDoc({ copies: o.value })} className={cn("h-7 flex-1 rounded-[3px] border text-[11px] font-semibold transition-colors", doc.copies === o.value ? "border-primary bg-primary text-primary-foreground" : "border-input bg-surface hover:bg-secondary")}>{o.label}</button>)}</div></div>
    ...
    <div className="flex flex-col gap-1"><span className="panel-label">Canhoto</span>{rules.canhoto === "required" ? <><Check label="Canhoto" checked disabled hint="obrigatório" /><Field label={`Altura do canhoto — ${Math.round(doc.stubRatio * 100)}%`}><input type="range" min={0.15} max={0.5} step={0.01} value={doc.stubRatio} onChange={(e) => setDoc({ stubRatio: parseFloat(e.target.value) })} className="accent-[var(--color-primary)]" /></Field></> : <p className="text-[11px] text-muted-foreground">Não aplicável a este tipo de bloco.</p>}</div>
    <div className="flex flex-col gap-2"><span className="panel-label">Tipografia</span><FontSelect label="Fonte da marca" value={doc.brandFont || doc.titleFont} onChange={(v) => setDoc({ brandFont: v })} /><FontSlider label="Tamanho da marca" value={doc.brandSize ?? 12} min={8} max={24} step={0.5} onChange={(v) => setDoc({ brandSize: v })} /><FontSelect label="Fonte dos títulos" value={doc.titleFont} onChange={(v) => setDoc({ titleFont: v })} /><FontSlider label="Tamanho dos títulos" value={doc.titleSize ?? 11} min={7} max={24} step={0.5} onChange={(v) => setDoc({ titleSize: v })} /><FontSelect label="Fonte do corpo" value={doc.bodyFont} onChange={(v) => setDoc({ bodyFont: v })} /><FontSlider label="Tamanho do corpo" value={doc.bodySize ?? 8} min={5} max={14} step={0.5} onChange={(v) => setDoc({ bodySize: v })} /><FontSlider label="Tamanho da tabela" value={doc.table.fontSize} min={5} max={14} step={0.5} onChange={(v) => setDoc({ table: { ...doc.table, fontSize: v } })} /></div>
  </div>;
}
