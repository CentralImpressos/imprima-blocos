...
export function BlockSettings() {
  ...
  const availableSizes = doc.typeId === "comanda"
    ? BLOCK_SIZES.filter((s) => s.widthMm < s.heightMm && s.id !== "14x10")
    : doc.typeId === "pedido"
  ...
  return <div className="flex flex-col gap-3 p-3">
    ...
    <div className="flex flex-col gap-1"><span className="panel-label">Vias</span><div className="grid grid-cols-3 gap-1">{COPIES_OPTIONS.map((o) => <button type="button" key={o.value} aria-pressed={doc.copies === o.value} onClick={(e) => { e.preventDefault(); e.stopPropagation(); setDoc({ copies: o.value }); }} className={cn("h-8 flex-1 rounded-[3px] border text-[11px] font-semibold transition-colors", doc.copies === o.value ? "border-primary bg-primary text-primary-foreground" : "border-input bg-surface hover:bg-secondary")}>{o.label}</button>)}</div></div>
