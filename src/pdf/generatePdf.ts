...
export async function imageAspect(dataUrl: string): Promise<number> {
  return new Promise((resolve) => { const img = new Image(); img.onload = () => resolve(img.naturalWidth / img.naturalHeight || 1); img.onerror = () => resolve(1); img.src = dataUrl; });
}
...
async function embedSelectedFonts(pdf: PDFDocument, doc: BlockDoc, regular: any, bold: any) {
  ...
  try { const bytes = await loadIconFont("solid"); const font = await pdf.embedFont(bytes); fonts.set(ICON_SOLID_FONT, { regular: font, bold: font }); } catch {}
  try { const bytes = await loadIconFont("brands"); const font = await pdf.embedFont(bytes); fonts.set(ICON_BRANDS_FONT, { regular: font, bold: font }); } catch {}
  return fonts;
}
