...
export function buildFooter(ctx: LayoutContext, bottomY: number): DocElement[] {
  ...
  const els: DocElement[] = []; let y = bottomY; const msg = f.message || company.footerText; if (msg) y -= ptToMm(f.fontSize + 1) + 1; if (items.length) y -= ptToMm(f.fontSize) + 1; els.push({ kind: "line", x1: m, y1: y - 2, x2: m + contentW, y2: y - 2, lineWidth: 0.3, gray: 0.6 });
