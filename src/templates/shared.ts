...
const iconFont = (icon: keyof typeof ICON_GLYPHS) => icon === "whatsapp" || icon === "instagram" || icon === "facebook" ? ICON_BRANDS_FONT : ICON_SOLID_FONT;
const iconAdvance = (size: number, icon: keyof typeof ICON_GLYPHS) => { const ratios = { whatsapp: 0.15, instagram: 0.15, facebook: 0.16, phone: 0.13, envelope: 0.15, globe: 0.15, location: 0.12 }; return size * (ratios[icon] ?? 0.14); };
function addIconText(els: DocElement[], x: number, y: number, size: number, icon: keyof typeof ICON_GLYPHS, text: string, fontFamily: string, gray = 0.3) { if (!text) return x; els.push({ kind: "text", x, y, size, text: ICON_GLYPHS[icon], gray, fontFamily: iconFont(icon) }); const nextX = x + iconAdvance(size, icon) + 1.1; els.push({ kind: "text", x: nextX, y, size, text, gray, fontFamily }); return nextX + text.length * size * 0.16 + 2.5; }
...
export function buildFooter(ctx: LayoutContext, bottomY: number): DocElement[] {
  ...
  if (f.showName && (company.tradeName || company.name)) items.push({ text: company.tradeName || company.name }); if (f.showAddress && fullAddress(company)) items.push({ icon: "location", text: fullAddress(company) }); if (f.showPhone && company.phone) items.push({ icon: "phone", text: company.phone }); if (f.showWhatsapp && company.whatsapp) items.push({ icon: "whatsapp", text: company.whatsapp }); if (f.showEmail && company.email) items.push({ icon: "envelope", text: company.email }); if (f.showWebsite && company.website) items.push({ icon: "globe", text: company.website }); if (f.showInstagram && company.instagram) items.push({ icon: "instagram", text: company.instagram }); if (f.showFacebook && company.facebook) items.push({ icon: "facebook", text: company.facebook }); if (f.showNotes && company.notes) items.push({ text: company.notes });
  const els: DocElement[] = []; let y = bottomY - 2; const msg = f.message || company.footerText; if (msg) y -= ptToMm(f.fontSize + 1) + 1; if (items.length) y -= ptToMm(f.fontSize) + 1; els.push({ kind: "line", x1: m, y1: y - 2, x2: m + contentW, y2: y - 2, lineWidth: 0.3, gray: 0.6 });
  if (items.length) { const sep = 4; const widths = items.map((item) => item.text.length * f.fontSize * 0.16 + (item.icon ? iconAdvance(f.fontSize, item.icon) + 1.1 : 0)); const totalW = widths.reduce((a, b) => a + b, 0) + sep * Math.max(0, items.length - 1); let x = m + Math.max(0, (contentW - totalW) / 2); items.forEach((item, index) => { if (item.icon) { els.push({ kind: "text", x, y, size: f.fontSize, text: ICON_GLYPHS[item.icon], gray: 0.25, fontFamily: iconFont(item.icon) }); x += iconAdvance(f.fontSize, item.icon) + 1.1; } els.push({ kind: "text", x, y, size: f.fontSize, text: item.text, gray: 0.25, fontFamily: doc.bodyFont }); x += item.text.length * f.fontSize * 0.16; if (index < items.length - 1) { els.push({ kind: "text", x, y, size: f.fontSize, text: "•", gray: 0.35, fontFamily: doc.bodyFont }); x += sep; } }); y += ptToMm(f.fontSize) + 1; }
  if (msg) els.push({ kind: "text", x: m, y, size: f.fontSize + 1, bold: true, align: "center", width: contentW, text: msg, fontFamily: doc.bodyFont }); return els;
}
export function footerHeight(ctx: LayoutContext): number { const f = ctx.doc.footer; return ptToMm(f.fontSize) + ptToMm(f.fontSize + 1) + 8; }
