...
export function buildFooter(ctx: LayoutContext, bottomY: number): DocElement[] {
  ...
  if (items.length) { const sep = 4; const widths = items.map((item) => item.text.length * size * 0.16 + (item.icon ? iconAdvance(size, item.icon) + 1.1 : 0)); const totalW = widths.reduce((a, b) => a + b, 0) + sep * Math.max(0, items.length - 1); let x = m + Math.max(0, (contentW - totalW) / 2); items.forEach((item, index) => { if (item.icon) { els.push({ kind: "text", x, y, size, text: ICON_GLYPHS[item.icon], gray: 0.25, fontFamily: iconFont(item.icon) }); x += iconAdvance(size, item.icon) + 1.1; } els.push({ kind: "text", x, y, size, text: item.text, gray: 0.25, fontFamily: doc.bodyFont }); x += item.text.length * size * 0.16; if (index < items.length - 1) { els.push({ kind: "text", x, y, size, text: "•", gray: 0.35, fontFamily: doc.bodyFont }); x += sep; }); y += ptToMm(size) + 1; }
...
function addIconText(els: DocElement[], x: number, y: number, size: number, icon: keyof typeof ICON_GLYPHS, text: string, fontFamily: string, gray = 0.3) { if (!text) return x; els.push({ kind: "text", x, y, size, text: ICON_GLYPHS[icon], gray, fontFamily: iconFont(icon), bold: iconFont(icon) === ICON_SOLID_FONT }); const nextX = x + iconAdvance(size, icon) + 1.1; els.push({ kind: "text", x: nextX, y, size, text, gray, fontFamily }); return nextX + text.length * size * 0.16 + 2.5; }
