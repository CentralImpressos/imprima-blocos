...
/** Cabeçalho comum: logo + nome + contato. Retorna elementos e o y final (mm). */
export function buildHeader(
  ctx: LayoutContext,
  opts: { title?: string; subtitle?: string; compact?: boolean } = {},
): { els: DocElement[]; y: number } {
  const { company, m, contentW } = ctx;
  const els: DocElement[] = [];
  let y = m;
  const compact = opts.compact ?? false;

  const logoH = (compact ? 10 : 14) * (company.logoScale || 1);
  let textX = m;
  let textW = contentW;
  ...
  if (sub && sub !== (company.tradeName || company.name)) {
    els.push({ kind: "text", x: textX, y, size: compact ? 6.5 : 7.5, text: sub, gray: 0.35 });
    y += ptToMm(7.5) + 0.6;
  }

  const contact = [company.phone, company.whatsapp].filter(Boolean).join("  •  ");
  const addr = fullAddress(company);
  if (addr) {
    els.push({ kind: "text", x: textX, y, size: 6.5, text: addr, gray: 0.3 });
    y += ptToMm(6.5) + 0.6;
  }
  if (contact) {
    els.push({ kind: "text", x: textX, y, size: 6.5, text: contact, gray: 0.3 });
    y += ptToMm(6.5) + 0.6;
  }
...
export function buildFooter(ctx: LayoutContext, bottomY: number): DocElement[] {
  const { company, doc, m, contentW } = ctx;
  const f = doc.footer;
  const parts: string[] = [];
  if (f.showName) parts.push(company.tradeName || company.name);
  if (f.showAddress) parts.push(fullAddress(company));
  if (f.showPhone && company.phone) parts.push(`Tel: ${company.phone}`);
  if (f.showWhatsapp && company.whatsapp) parts.push(`WhatsApp: ${company.whatsapp}`);
  if (f.showEmail && company.email) parts.push(company.email);
  if (f.showWebsite && company.website) parts.push(company.website);
  if (f.showInstagram && company.instagram) parts.push(company.instagram);
  if (f.showFacebook && company.facebook) parts.push(company.facebook);
  if (f.showNotes && company.notes) parts.push(company.notes);
