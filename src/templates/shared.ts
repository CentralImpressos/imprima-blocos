...
export function buildHeader(ctx: LayoutContext, opts: { title?: string; subtitle?: string; compact?: boolean } = {}): { els: DocElement[]; y: number } {
  ...
  if (addr) { els.push({ kind: "text", x: textX, y, size: 6.5, text: addr, gray: 0.3, fontFamily: doc.bodyFont }); y += ptToMm(6.5) + 0.6; }
  if (contact) { els.push({ kind: "text", x: textX, y, size: 6.5, text: contact, gray: 0.3, fontFamily: doc.bodyFont }); y += ptToMm(6.5) + 0.6; }
