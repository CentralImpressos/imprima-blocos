...
export const renderPedido: TemplateRenderer = (ctx: LayoutContext) => {
  ...
  fields.forEach((label, i) => { const x = m + i * (third + 4.5); els.push({ kind: "text", x, y, size: s, bold: true, text: label }); if (i !== 1) els.push({ kind: "line", x1: x + label.length * s * 0.17 + 1, y1: y + ptToMm(s) + 0.5, x2: x + third, y2: y + ptToMm(s) + 0.5, lineWidth: 0.3 }); });
  y += ptToMm(s) + 4;
  ...
  els.push(
    ...
    { kind: "text", x: m + contentW / 2 + 2, y: ty + (totalH - ptToMm(totalSize)) / 2, size: totalSize, bold: true, text: doc.fields["totalLabel"] || "TOTAL R$" },
  );
